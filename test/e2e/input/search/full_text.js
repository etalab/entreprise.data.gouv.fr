...
module.exports = {
  'Display list of etablissements': function (browser) {
    ...
    browser.assert.urlEquals(browser.launch_url + 'search?fullText=' + cocaCola.search + '&page=1')
    // Some results are present
    browser.expect.element( listResults + ' > li:first-of-type > .panel').to.be.visible
    // First result should be main corporation
    browser.expect.element(listResults + ' > li:first-of-type > .panel > h4').text.to.contain(cocaCola.firstResultName)
  ...
  'Click on etablissement goes to etablissement card': function (browser) {
    ...
    browser.assert.urlEquals(browser.launch_url + 'etablissement/' + cocaCola.firstResultSiret)
    browser.end()
  }
}
